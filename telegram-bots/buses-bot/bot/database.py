...
stop id, a route id and an operator. Instead every button stores its real
...
CREATE TABLE IF NOT EXISTS users (
    ...
    operator       TEXT    NOT NULL DEFAULT 'rapid-bus-kl',
...
-- Remembers which vehicle we already alerted on, so a bus creeping toward a
-- stop produces one message instead of one per poll.
...
class Database:
    ...
    async def make_callback(
        ...
        await self._execute(
            "INSERT INTO callbacks (token, action, payload, user_id, created_at) VALUES (?, ?, ?, ?, ?)",
