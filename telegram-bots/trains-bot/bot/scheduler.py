...
Jobs are deduplicated by a natural key (user, station, line, departure) so a
...
from .gtfs import GTFSManager, has_live, haversine_m
...
# How long before a live alert may repeat for the same train and user.
LIVE_ALERT_COOLDOWN = 20 * 60
# A train within this distance of a favourite station triggers an alert.
# Rail corridors are long, so this is wider than the bus equivalent.
LIVE_ALERT_RADIUS_M = 2500
# A feed silent for longer than this is treated as stale.
FEED_STALE_AFTER = 3 * 3600
# Rapid KL headway expansion produces a departure every few minutes. Without a
# cap, one favourite there would queue hundreds of reminders an hour.
MAX_REMINDERS_PER_LINE = 6
...
class Scheduler:
    ...
    async def _run_departure(self, job: Any, payload: dict[str, Any]) -> None:
        ...
        doc = departure_reminder_doc(
            stop_name=payload.get("stop_name", "your station"),
            route_name=payload.get("route_name", "Your train"),
    ...
    async def _run_digest(self, job: Any, payload: dict[str, Any]) -> None:
        ...
        for fav in favourites:
            ...
            if fav["route_id"]:
                route_ids = [fav["route_id"]]
            else:
                route_ids = [r.route_id for r in feed.routes_at_stop(fav["stop_id"])]

            for route_id in route_ids:
                times = feed.departures(fav["stop_id"], route_id, day)
                if not times:
                    continue
    ...
    async def _plan_departures(self) -> None:
        ...
        for sub in subs:
            ...
            if sub["route_id"]:
                route_ids = [sub["route_id"]]
            else:
                route_ids = [r.route_id for r in feed.routes_at_stop(sub["stop_id"])]
            ...
            for route_id in route_ids:
                ...
                for departure in feed.departures(sub["stop_id"], route_id, day):
                    if queued >= MAX_REMINDERS_PER_LINE:
                        break
    ...
    async def _check_live_alerts(self) -> None:
        # Only operators with a realtime feed can raise a proximity alert, so a
        # Rapid KL favourite is skipped rather than polled pointlessly.
        subs = [
            sub
            for sub in await self.db.subscriptions_of_kind("live")
            if sub["favourite_id"] is not None
            and sub["stop_id"]
            and has_live(sub["operator"])
        ]
        if not subs:
            return
        ...
        vehicles_by_operator = {
            operator: await self.gtfs.get_vehicles(operator, max_age=45)
        ...
        for sub in subs:
            ...
            for vehicle in vehicles_by_operator.get(sub["operator"], []):
                ...
                await self.bot.send(
                    user["chat_id"],
                    live_alert_doc(
                        route.display if route else (vehicle.route_id or "A train"),
                        stop.display,
                        distance,
                        vehicle,
                    ),
