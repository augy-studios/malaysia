...
Every user-facing message this bot sends is a Rich Message
(https://core.telegram.org/bots/features#rich-messages). Telethon speaks
MTProto, and `sendRichMessage` only exists on the HTTP Bot API, so there is no
Telethon method for it. A bot token is valid on both transports at once, so the
approach here is:
...
them. Every send and edit in this project goes through `RichSender`, so both
the rich path and its degradation are handled in exactly one place and no
handler is able to bypass them by calling Telethon directly.
...
def _button_payload(btn: Any) -> dict[str, Any] | None:
    ...
    Telethon has moved this around between versions. On 1.44 a
...
class RichSender:
    ...
    async def send(
        ...
        """Send `doc` to `chat_id` as a Rich Message."""
