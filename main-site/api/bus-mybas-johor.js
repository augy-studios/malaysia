/*
  Proxies the myBAS Johor Bahru (Causeway Link) GTFS-static feed from
  data.gov.my. The upstream endpoint 302-redirects to a real GTFS zip bundle
  (~4.9MB). routes.txt and stops.txt are small, plain CSV, so we unzip and
  parse just those two on every request (no npm dependencies are configured
  in this project - ZIP central directory + DEFLATE are handled with Node's
  built-in zlib) and hand the client clean JSON instead of a binary blob or a
  bare download link. Vercel's edge cache (see Cache-Control below) keeps
  this from re-downloading/re-unzipping the full bundle on every visitor.
*/
const zlib = require('zlib');

const UPSTREAM_URL = 'https://api.data.gov.my/gtfs-static/mybas-johor';
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(isAbort ? 504 : 500).json({
      success: false,
      error: isAbort ? 'Request to upstream timed out.' : ((err && err.message) || 'Unknown error fetching myBAS Johor feed.')
    });
  }
};
