// Vercel serverless function - read-through proxy for Prasarana bus live vehicle
// positions via data.gov.my (GTFS-realtime). The upstream genuinely returns raw
// GTFS-realtime protobuf (application/octet-stream, a serialised FeedMessage),
// not JSON - confirmed by inspecting the raw bytes, which decode cleanly against
// the standard gtfs-realtime.proto field layout (FeedHeader/FeedEntity/
// VehiclePosition/TripDescriptor/Position/VehicleDescriptor). There is no
// package.json in this repo, so rather than add an npm protobuf dependency we
// decode the (small, well-known) wire format by hand below and hand the client
// plain JSON.
...
// Parses a length-delimited protobuf message into a list of {field, wire, raw}
// entries. `raw` is a Uint8Array for wire types 2/1/5 (caller interprets it),
// or a number for wire type 0 (varint).
function parseFields(bytes, start, end) {
  ...
  while (pos < end) {
    ...
    if (wireType === 0) {
      ...
      // Unsupported/unexpected wire type - bail out rather than risk an infinite loop.
...
module.exports = async (req, res) => {
  ...
  const upstreamUrl = `https://api.data.gov.my/gtfs-realtime/vehicle-position/prasarana/?category=${encodeURIComponent(category)}`;
  ...
  try {
    ...
    res.status(isAbort ? 504 : 500).json({
      ...
      error: isAbort
        ...
        : "Failed to reach upstream Prasarana live position feed.",
    });
  }
};
